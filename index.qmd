...
**Result:** \$100M investment in the right project = Massive hit
:::
:::
...
3.  **Transferable Skills** - One technique, infinite applications
4.  **The Future** - AI is changing everything
...
-   Recovery pattern: Angry Rock → Sad Songs → Chill Vibes → Happy Hits
:::
...
People search for flu symptoms BEFORE going to the doctor
...
**PLUS:** ambient coughing sound

<!-- **Google: Real-time detection** -->

:::

::: {.column width="50%"}
```{r flu-trends}
# Simulate flu detection comparison
weeks <- 1:52
set.seed(321)

flu_data <- tibble(
  week = rep(weeks, 3),
  source = rep(c("Actual Cases", "Google Trends", "CDC Reports"), each = length(weeks))
) %>%
  mutate(
    cases = case_when(
      source == "Actual Cases" ~ 100 + 400 * dnorm(week, 8, 4) * 50 + rnorm(n(), 0, 10),
      source == "Google Trends" ~ 100 + 400 * dnorm(week, 7.7, 4) * 50 + rnorm(n(), 0, 15),
      source == "CDC Reports" ~ 100 + 400 * dnorm(week, 9, 4) * 50 + rnorm(n(), 0, 10)
    )
  )

p <- plot_ly(flu_data, x = ~week, y = ~cases, color = ~source,
             type = 'scatter', mode = 'lines', height = 400,
             colors = c("#e74c3c", "#3498db", "#95a5a6"),
             line = list(width = 4)) %>%
  layout(
    title = list(text = "Flu Detection Speed: Google vs CDC", font = list(size = 16)),
    xaxis = list(title = "Week of Year", gridcolor = 'rgba(128,128,128,0.1)'),
    yaxis = list(title = "Relative Cases", gridcolor = 'rgba(128,128,128,0.1)'),
    hovermode = 'x unified',
    plot_bgcolor = '#f8f9fa',
    paper_bgcolor = '#f8f9fa',
    font = list(family = "Arial, sans-serif"),
    legend = list(bgcolor = 'rgba(255,255,255,0.8)', x = 0.7, y = 0.95),
    annotations = list(
      list(x = 2.5, y = 1000,
           text = "Google detects<br>~1 week earlier",
...
::: {.callout-note}
## Impact: Early detection = Better preparation for healthcare systems
:::

## Walmart's Hurricane Analytics {.smaller}

```{r walmart-products}
# Products that surge before hurricanes
products <- tibble(
  product = c("Strawberry Pop-Tarts", "Beer", "Flashlights", 
              "Batteries", "Bottled Water", "Bread"),
  sales_increase = c(700, 350, 250, 220, 180, 150),
  surprise_factor = c(95, 40, 20, 15, 10, 10)
)

p1 <- plot_ly(products, x = ~reorder(product, sales_increase), y = ~sales_increase,
              type = 'bar', height = 380, marker = list(
                color = ~surprise_factor,
                colorscale = 'Reds',
                showscale = TRUE,
                colorbar = list(title = "Surprise<br>Factor", titlefont = list(size = 12)),
                line = list(width = 1.5, color = '#ffffff')
              ),
              text = ~paste0("+", sales_increase, "%"),
              textposition = 'outside',
              textfont = list(size = 13)) %>%
  layout(
    title = list(text = "Product Sales Increase Before Hurricanes", font = list(size = 16)),
    xaxis = list(title = "", tickfont = list(size = 12)),
    yaxis = list(title = "Sales Increase (%)", gridcolor = 'rgba(128,128,128,0.1)'),
    plot_bgcolor = '#ffffff',
    paper_bgcolor = '#ffffff',
...
People want **comfort food** that doesn't need refrigeration
...
# 🔄 Transferable Techniques {background-color="#9b59b6"}
...
**How it works:** - Analyzes millions of purchase histories - Finds patterns in co-purchasing behavior - Recommends based on similarity

**Business Value:** - **35% of Amazon's revenue** from recommendations - Increases average order value by **20-30%** - Improves customer experience
:::
...
**How it works:** - Analyzes thousands of clinical cases - Matches symptoms to diagnoses (92% accuracy) - Suggests treatments with best outcomes

**Business Value:** - Reduces diagnostic errors - Faster, evidence-based decisions - Lower costs through fewer unnecessary tests - Better patient outcomes
:::
...
**How it works:** - Analyzes profiles of successful employees - Compares candidate characteristics - Ranks by similarity to top performers

**Business Value:** - **40% higher retention** for AI-matched hires - Reduces time-to-hire from months to weeks - Better culture fit - Removes unconscious bias
:::
...
**How it works:** - Analyzes transaction patterns in real-time - Compares to user's behavior history - Flags anomalies above risk threshold

**Business Value:** - **95%** fraud detection rate - **0.1%** false positive rate - Saves banks **\$billions** annually - Protects customers in real-time
:::
...
**How it works:** - Processes text from earnings calls - Extracts sentiment (positive/negative/neutral) - Combines with price & volume data

**Business Value:** - Real-time sentiment tracking - Predicts stock movements - Informs trading decisions - Quantifies qualitative data
:::
...
**Business Value:** - Target marketing campaigns - Personalize offers - Optimize pricing strategies - Allocate resources efficiently
:::
:::
:::
...
**Business Value:** - Faster diagnosis - Personalized treatment plans - Drug development targeting - Better patient outcomes
:::
:::
:::
...
**Business Value:** - Identify market opportunities - Benchmark against competitors - Strategic positioning - Investment decisions
:::
:::
:::
...
**Business Value:** - Reduces file size by 80-90% - Faster image loading - Lower bandwidth costs - Maintains visual quality
:::
:::
:::
:::

# 🔮 The AI Revolution {background-color="#2c3e50"}
